...
The first thing to note is that this sample is pretty similar in overall diversity to the healthy samples in our database. (Figure \@ref(fig:CTOverview))
...
psmr::mhg_outlier(psmr::mhg_range_for(ct.gut.genus),psmr::mhg_range_for(people.healthy.gut.genus)) %>% filter(original>max) %>% knitr::kable()
...
cb.gut.genus <- subset_samples(people.norm, Username == "cb")
...
t <- psmr::unique_taxa(subset_samples(nj1, Date > "2017-07-01"), subset_samples(nj1, Date < "2017-07-01")) %>% select(abundance = 1,tax_name)  

data.frame(tax_name = t$tax_name,abundance = t$abundance/10000) %>% knitr::kable(digits = 3)
```
...
s_cf <- psmr::mhg_outlier(psmr::mhg_range_for(ed.norm),psmr::mhg_range_for(people.healthy.gut.genus))

s_cf2 <- s_cf %>% select(-taxa) /10000

data.frame(taxa = s_cf$taxa, s_cf2) %>% knitr::kable(digits = 3, caption = "Ed outlier abundances (%)")
...
s_cf <- mhg_outlier(mhg_range_for(sj.genus),mhg_range_for(people.healthy.gut.genus)) %>% filter(original>max) 

s_cf2 <- s_cf %>% select(-taxa) /10000

data.frame(taxa = s_cf$taxa, s_cf2) %>% knitr::kable(digits = 3, caption = "SJ outlier abundances (%)")
...
s_cf <- mhg_outlier(mhg_range_for(sj.species),
                    mhg_range_for(subset_samples(people.species,
                                                 Condition == "Healthy" & Site == "gut"))) %>% filter(original>max) 
...
s_cf <- mhg_outlier(mhg_range_for(c),
                    mhg_range_for(people.healthy.gut.genus)) %>% filter(original>max) 
