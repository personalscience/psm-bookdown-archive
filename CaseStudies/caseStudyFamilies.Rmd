...
```{r}
mhg_unique(ub16,ub16.mom)[1]/10000
```

and the genus found in the mother but not the daughter

```{r}
mhg_unique(ub16.mom,ub16)[1]/10000
```

Here are species in the daughter but not the mom:

```{r}
mhg_unique(ub16.species,ub16.mom.species)[1]/10000
```

and vice versa:

```{r}
mhg_unique(ub16.mom.species,ub16.species)[1]/10000
```
...
```{r}
#mhg_unique(subset_samples(ub3.species, Date > "2016-06-01"),subset_samples(bird.species,Username == "ub16b"))[-2]/10000
```
...
kable(mhg_unique(ktp,kp)[1]/10000, col.names = c( "Wife (not husband)%"))
kable(mhg_unique(kp,ktp)[1]/10000, col.names = c( "Husband (not wife)%"))
```
