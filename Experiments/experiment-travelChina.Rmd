## Traveling to China {#chinaTravel}
...
Here's an overall heat plot of my gut, day-to-day before and after the trip \@ref(fig:experimentChinaHeatGut)

```{r experimentChinaHeatGut, fig.cap =  "Gut samples before/during/after a trip to Beijing from the US. Dark is 0, lighter colors are higher abundance"}

chinaDates <- rikStats %>% dplyr::filter(Geo=="Beijing") #%>% select(Date) %>% as.vector()  #%in% sample_data(gut.norm)$Date # == as.Date(sample_data(gut.norm)$Date)
...
But the obvious standout is the genus _Coprobacter_, which soared beginning a few days after arrival and settled back after my return. I looked in my other samples over the long term and find that it is strongly associated with my China trip. \@ref(fig:travelChinaPlotCopro)


```{r travelChinaPlotCopro, fig.cap="Changes in gut microbiome abundance of _Coprobacter_ over time. Area shaded in red is the period while traveling from the U.S. to China."}
...
How about diversity? Did that change? (Figure \@ref(fig:travelChinaPlotDiversityGut))

```{r travelChinaPlotDiversityGut, fig.cap = "Shannon diversity of gut samples during a week-long trip to Beijing."}
...
You can see that some sites may have changed more than others. (Figure \@ref(fig:travelChinaPlotDiversitySites))
```{r travelChinaPlotDiversitySites, fig.cap = "Shannon diversity of all samples during a week-long trip to Beijing."}
...
```{r travelChinaPlotDiversityskin}
...
Let's look at the other sites I tested.  Did anything unusual happen in my skin microbiome? As usual, I look first at the overall heatmap to spot any obvious changes.

```{r travelChinaSkin, fig.cap = "Heat map of my skin microbiome before and after a trip to China."}
...
```{r travelChinaRhizo, fig.cap = "Long-term abundance of Rhizobium in my skin microbiome. Red line indicates the period of traveling to China."}
