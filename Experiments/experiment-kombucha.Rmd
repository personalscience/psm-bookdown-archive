...
For healthy bacteria-rich drinks that affect the microbiome, many people immediately think of kombucha. Served chilled during the summer, it has a well-deserved reputation as a natural refreshing alternative to soft drinks. Despite its tangy, mildly sweet taste, it has a surprisingly low amount of sugar: less than two grams in a serving^[Though [one large test](https://www.bevnet.com/news/2016/kombucha-study-raises-sugar-content-questions) by the BevNet industry trade site says the labels may under-report the real amount], compared to more than 20 grams in the same amount of orange juice or 39 grams in a can of Coke.
...
I tested my gut microbiome each day, as well as my mouth and skin microbiome at intervals during the experiment and sure enough, the _Bacillus_ shows up loud and clear. (Figure \@ref(fig:experimentKombuchaPlotBacillus) )

```{r experimentKombuchaPlotBacillus, fig.cap = "The blue line represents days I drank 6 servings of kombucha."}
...
Let's look at that order _Rhodospirillales_ that contains the genus _Acetobacter_ found in the SCOBY. (Figure \@ref(fig:experimentKombuchaRhodo))

```{r experimentKombuchaRhodo, fig.cap="Abundance of microbes that include the genus Acetobacter found in the SCOBY"}
...
```{r kombuchaExperimentPlots, echo = FALSE, message = FALSE, warning = FALSE}
...
Finally, let's look at the levels of a few "probiotic" microbes, including the one listed on the label (Figure \@ref(fig:kombuchaAllProbiotics))

```{r kombuchaAllProbiotics, fig.cap = "Abundance of key 'probiotic' microbes while consuming kombucha."}
...
For comparison, let's look at a longer time horizon (Figure \@ref(fig:kombuchaPlotLong))

```{r kombuchaPlotLong, fig.cap = "Daily abundance of key microbes while drinking kombucha (blue lines). Blank regions are days when I have no data."}
