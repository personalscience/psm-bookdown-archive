...
# uncomment the following lines to graph without dependency on phyloseq
# g1 <- sprague_genus %>% filter(tax_name == "Leuconostoc" & site == "gut" &
#                            date %in% seq(from=fromDate,to=toDate, by=1)) %>% 
#   ggplot(aes(x=date,y=percent)) + geom_bar(stat = "identity") +
#   ggtitle("Kefir Taxa Abundance (%)") + scale_y_continuous(labels=function(x)x) + ylab("Abundance (%)") + 
...
# g1 + geom_point(aes(x=g1$data$date,y=-Inf),color="red",alpha=0.3,size=1)
