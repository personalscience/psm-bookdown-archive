...
kable(mhg_abundance(evac1, top = 10)/10000, digits=2,col.names=c("Middle", "Blended"))
```
...
kable(mhg_abundance(evac2, top = 10)/10000, digits=2,col.names=c("First", "Second"))
```
...
kable(mhg_abundance(evac3, top = 10)/10000, digits=2,col.names=c("Day 1 Morning", "Day 1 Afternoon"))
kable(mhg_abundance(evac4, top = 10)/10000, digits=2,col.names=c("Day 2 Morning", "Day 2 Afternoon"))
kable(mhg_abundance(evac5, top = 10)/10000, digits=2,col.names=c("Day 3 Morning", "Day 3 Afternoon"))
...
kable(mhg_abundance(person2, top = 10)/10000, digits=2,col.names=c("Sameday1", "Sameday 2"))
...
kable(data.frame(estimate_richness(evac1, measures = "Shannon"), location= c("Blended","Middle")), col.names = c("Diversity","Location"), row.names = FALSE)
```

---

Scientists at uBiome [recently released](https://www.biorxiv.org/content/early/2018/05/15/322677) results from experiments testing the variability of gut samples.  My own experiments show considerable day-to-day variability, so I was interested to see their conclusions, which are based on much more rigorous testing.
...
knitr::kable(data_frame(DescTools::CCC(p0.fb$Ratio1,p0.fb$Ratio2)[["rho.c"]][,"est"],
             DescTools::CCC(p0.fb$Firmicutes1,p0.fb$Firmicutes2)[["rho.c"]][,"est"],
             DescTools::CCC(p0.fb$Bacteroidetes1,p0.fb$Bacteroidetes2)[["rho.c"]][,"est"]),
...
clinical <- readr::read_csv("../assets/clinicalTaxa2016.csv")
clinical_genus <- (clinical %>% dplyr::filter(tax_rank=="genus"))$tax_name
...
dupes_dates <- sort(c(paste0("sample",stringr::str_pad(seq(1,25,1),width=2, pad = "0"),"A"),
                      paste0("sample",stringr::str_pad(seq(1,25,1),width=2, pad = "0"),"B")))
