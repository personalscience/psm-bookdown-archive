...
```{r mouthBeforeAfter1}
kable(head(mhg_abundance(d1)/10000,10), col.names = c("% before","% after"))

#mhg_abundance(prune_taxa(taxa_sums(d.before1)>0,d.before1))
#mhg_abundance(prune_taxa(taxa_sums(d.after1)>0,d.after1))

#mhg_unique(d.after1,d.before1)[1]  # this is NULL, meaning that I gained nothing new
...
```{r mouthUnique}
kable(mhg_unique(d.before1,d.after1)[1]/10000, , col.names = "% before")
```
...
```{r mouthBeforeAfter2}
kable(head(mhg_abundance(d2)/10000,10), col.names = c("% before","% after"))

#mhg_abundance(prune_taxa(taxa_sums(d.before2)>0,d.before2))
#mhg_abundance(prune_taxa(taxa_sums(d.after2)>0,d.after2))

# mhg_unique(d.after2,d.before2)[1]  # Yup, null this time too.
```
...
```{r beforeAfterDentist2}
kable(mhg_unique(d.before2,d.after2)[1]/10000, col.names = "% before")
```
