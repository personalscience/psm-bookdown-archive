...
Figure \@ref(fig:cleansePlot) is a broad, phylum-level look at how the various microbes shifted in abundance. As you can see, all of these high-level colonies were back to the same proportions that had been before the cleanse.
...
```{r cleansePlotGenus, fig.cap="Overall Genus-level summary, from baseline (2 weeks before the cleanse) to CC (colon cleanse) to one month after CC."}
# cleanseDates <- sample_data(gut.norm) %>% filter("CC" %in% Label) #%>% select(Date) %>% as.vector()  #%in% sample_data(gut.norm)$Date # == as.Date(sample_data(gut.norm)$Date)
...
My overall gut diversity spiked the day of the cleanse and then plunged the following day, but soon it was right back to normal (Figure \@ref(fig:coloncleanseDiversity))

```{r coloncleanseDiversity, fig.cap = "Shannon level diversity measures before and after the cleanse period."}
...
Of course, I'm not the first to study microbiome changes after a colon cleanse. A 2015 European study found increases in _Dorea_, which interestingly I found as well (Figure \@ref(fig:coloncleanseDorea)) .^[Read a detailed description of what microbes are noted in a bowel cleansing: @jalanka_effects_2015 ]
 
```{r coloncleanseDorea, fig.cap = "A rise in Dorea after a colon cleanse."}
