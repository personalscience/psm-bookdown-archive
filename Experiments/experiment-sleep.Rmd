...
mhg_taxa(gut.norm, "Bifidobacterium") %>% dplyr::filter(date > "2017-03-15" & date < "2017-04-15") %>% ggplot(aes(x=date,y=abundance/10000)) +  geom_point(aes(x = date, y = 0), color = "red") + 
  ...
  labs(title = "Potato Starch and Bifidobacterium", subtitle = "(Blue lines are days I took some potato starch)", y = "Bifidobacterium (%)") + theme(axis.title.y.right = element_text(color = "brown"), axis.text.y.right = element_text(color = "brown"))
